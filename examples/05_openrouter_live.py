...
"""05 — OpenRouter live: real OpenRouter stream via the OpenAI-compatible API.

OpenRouter exposes the OpenAI Chat Completions wire format, so the existing
`eager-tools-openai` adapter works unchanged — only `base_url` and `api_key`
differ. Pick any tool-capable model from https://openrouter.ai/models.

Requires: OPENROUTER_API_KEY in env, `pip install openai`.
Run:

    OPENROUTER_API_KEY=sk-or-v1-... python examples/05_openrouter_live.py
"""
...
async def main() -> None:
    ...
    print(f"{_ts()} requesting OpenRouter stream  model={DEFAULT_MODEL} …")
